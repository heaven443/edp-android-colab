...
class UserRepository(
    ...
    suspend fun login(email: String, password: String): AppResult<User> = safeCall {
        ...
        val found = matches.firstOrNull { 
            it.email?.equals(email.trim(), ignoreCase = true) == true && it.password == password 
        }
        if (found == null) AppResult.Failure.WrongLogin
        else AppResult.Success(found.toDomain())
    }
    ...
    suspend fun register(
        ...
        val taken = findUsers(email.trim()).any { 
            it.email?.equals(email.trim(), ignoreCase = true) == true 
        }
        if (taken) AppResult.Failure.EmailTaken
        else {
